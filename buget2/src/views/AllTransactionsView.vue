...
import { transactions, selectedCurrency, categories, blacklistedCategories } from '../stores/useStore'
...
const showBlacklisted = ref(true)

const totalAmount = computed(() => {
  const transactions = Object.values(groupedTransactions.value).flat()
  
  // Filter based on blacklist setting
  const filtered = transactions.filter(tx => {
    if (!tx.category) return true
    if (showBlacklisted.value) return true
    return !blacklistedCategories.value.includes(tx.category)
  })
  
  return filtered.reduce((sum, tx) => sum + tx.amount, 0)
})

const toggleBlacklisted = () => {
  showBlacklisted.value = !showBlacklisted.value
}
